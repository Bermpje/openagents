...
<body>
    ...
    def _require_admin(self, request) -> bool:
        ...
        if hasattr(self.network_instance, 'identity_manager'):
            identity = self.network_instance.identity_manager.active_agents.get(agent_id)
            if identity and identity.agent_group == 'admin':
                logger.info(f"Admin access granted for agent: {agent_id}")
                return True
            else:
                group = identity.agent_group if identity else 'unknown'
                logger.warning(f"Admin check failed: agent {agent_id} is in group '{group}', not 'admin'")
                return False
        else:
            logger.warning("Admin check failed: identity_manager not available")
            return False
